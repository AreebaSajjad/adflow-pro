...
import { CreditCard, CheckCircle, AlertCircle, Upload, Clock, XCircle } from "lucide-react";
...
export default function PaymentsPage() {
  ...
  const [form, setForm] = useState({ ad_id: "", amount: "", method: "", transaction_ref: "", sender_name: "", screenshot_url: "" });
  ...
  async function fetchData(userId: string) {
    ...
    const [adsRes, paymentsRes] = await Promise.all([
      supabase.from("ads").select("*, packages(name, price)").eq("user_id", userId).eq("status", "payment_pending"),
      supabase.from("payments").select("*, ads(title)").eq("user_id", userId).order("created_at", { ascending: false }),
    ]);
    setPendingAds(adsRes.data || []);
    setPayments(paymentsRes.data || []);
    // Auto-select first pending ad
    if (adsRes.data?.length) {
      const first = adsRes.data[0];
        ...
        <div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(59,110,240,0.15)", background: "rgba(13,18,32,0.8)" }}>
            <div className="px-6 py-4" style={{ borderBottom: "1px solid rgba(59,110,240,0.1)" }}>
              <h2 className="font-semibold text-white flex items-center gap-2"><CreditCard size={16} className="text-blue-400" /> Payment Details</h2>
            </div>
            <div className="p-6">
              {success && (
                <div className="flex items-start gap-2 px-4 py-3 rounded-xl mb-5 text-sm" style={{ background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.25)", color: "#34d399" }}>
                  <CheckCircle size={16} className="flex-shrink-0 mt-0.5" /> {success}
                </div>
              )}
              {error && (
                <div className="flex items-center gap-2 px-4 py-3 rounded-xl mb-5 text-sm" style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)", color: "#f87171" }}>
              ...
              {pendingAds.length === 0 ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-slate-600 text-sm mt-1">All your ads are either live or in review</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-sm text-slate-400 mb-2">Select Ad *</label>
                    <select value={form.ad_id} onChange={e => {
                      ...
                      {pendingAds.map(a => <option key={a.id} value={a.id}>{a.title} — PKR {a.packages?.price?.toLocaleString()}</option>)}
                    </select>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select value={form.method} onChange={e => update("method", e.target.value)} className="w-full px-4 py-3 rounded-xl text-sm input-dark cursor-pointer">
                  ...
                  <button type="submit" disabled={submitting} className="w-full py-3.5 rounded-xl font-semibold btn-primary flex items-center justify-center gap-2 disabled:opacity-60">
                    {submitting ? <span className="w-5 h-5 rounded-full border-2 border-white/30 border-t-white animate-spin" /> : <><Upload size={16} /> Submit Payment Proof</>}
                  </button>
                </form>
              )}
            </div>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(59,110,240,0.15)", background: "rgba(13,18,32,0.8)" }}>
            ...
            {loading ? (
              <div className="p-6 space-y-4">{Array(4).fill(0).map((_, i) => <div key={i} className="shimmer h-16 rounded-xl" />)}</div>
            ) : payments.length === 0 ? (
                    ...
                    <div key={p.id} className="px-6 py-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-slate-700 text-xs mt-0.5">{new Date(p.created_at).toLocaleDateString("en-PK", { day: "numeric", month: "short", year: "numeric" })}</p>
                        </div>
                        <span className="px-2.5 py-1 rounded-lg text-xs font-medium flex-shrink-0" style={{ background: ps.bg, color: ps.color }}>
                          {ps.label}
                        </span>
                      </div>
                    </div>
