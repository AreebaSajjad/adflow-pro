          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[1,2,3].map(i => (
              <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl p-6 animate-pulse">
                <div className="h-6 bg-gray-800 rounded mb-4 w-24"></div>
                <div className="h-10 bg-gray-800 rounded mb-2 w-32"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {packages.map((pkg, i) => (
              <div key={pkg.id} className={`rounded-xl p-6 border ${i === 1 ? 'border-blue-500 bg-blue-950/40' : 'border-gray-800 bg-gray-900'}`}>
                {i === 1 && (
                  <span className="text-xs bg-blue-600 text-white px-2 py-0.5 rounded-full mb-3 inline-block">Most Popular</span>
                )}
                ...
                <ul className="mt-4 space-y-2 text-sm text-gray-400">
                  <li className="flex items-center gap-2">
                    ...
                    {pkg.weight}x visibility weight
                  </li>
                  <li className="flex items-center gap-2">
                    <span className={pkg.homepage_visibility ? 'text-green-400' : 'text-red-400'}>
                      {pkg.homepage_visibility ? '✓' : '✗'}
                    </span>
                    Homepage visibility
                  </li>
                  <li className="flex items-center gap-2">
                    <span className={pkg.is_featured ? 'text-green-400' : 'text-red-400'}>
                      {pkg.is_featured ? '✓' : '✗'}
                    </span>
                    Featured placement
                  </li>
                  {pkg.refresh_days && (
                    <li className="flex items-center gap-2">
                      <span className="text-green-400">✓</span>
                      Auto refresh every {pkg.refresh_days} days
                    </li>
                  )}
                </ul>
                
                 <a href="/dashboard/client/ads/new"
                  className={`mt-6 block text-center py-2 rounded-lg text-sm font-medium transition ${i === 1 ? 'bg-blue-600 hover:bg-blue-500 text-white' : 'border border-gray-700 hover:border-gray-500 text-gray-300'}`}
                >
                  Get Started
                </a>
              </div>
