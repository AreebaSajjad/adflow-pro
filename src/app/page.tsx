    ...
    <div className="min-h-screen" style={{ background: "#0a0d14" }}>
      ...
      <section className="relative pt-28 pb-20 overflow-hidden hero-gradient grid-pattern">
        ...
        <div className="max-w-7xl mx-auto px-6 text-center relative">
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold text-white mb-6 leading-tight">
            Buy & Sell With<br />
            <span className="gradient-text">Full Confidence</span>
          </h1>
          ...
          <form onSubmit={handleHeroSearch} className="max-w-2xl mx-auto mb-8">
            <div className="relative flex items-center">
              ...
              <input
                type="text"
                placeholder="Search electronics, cars, property..."
                value={heroSearch}
                onChange={e => setHeroSearch(e.target.value)}
                className="w-full pl-12 pr-36 py-4 rounded-2xl text-sm input-dark"
                style={{ background: "rgba(20,27,45,0.9)", border: "1px solid rgba(59,110,240,0.25)", fontSize: "15px" }}
              />
              <button type="submit" className="absolute right-2 px-6 py-2.5 rounded-xl text-sm font-semibold btn-primary">
                Search
              </button>
            </div>
          </form>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {["Electronics", "Vehicles", "Property", "Jobs", "Fashion"].map(cat => (
              <Link key={cat} href={`/explore?category=${cat.toLowerCase()}`}
                ...
                style={{ background: "rgba(59,110,240,0.08)", border: "1px solid rgba(59,110,240,0.15)" }}>
                {cat}
              </Link>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {TRUST_STATS.map((s, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-blue-400 flex-shrink-0" style={{ background: "rgba(59,110,240,0.12)" }}>
                  {s.icon}
                </div>
                <div>
      ...
      <section className="py-16" style={{ background: "rgba(12,16,26,0.6)" }}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/explore" className="flex items-center gap-1 text-sm text-blue-400 hover:text-blue-300 transition-colors">
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {Array(8).fill(0).map((_, i) => <AdCardSkeleton key={i} />)}
            </div>
          ) : ads.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div className="text-center py-20">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: "rgba(59,110,240,0.12)" }}>
      ...
      <section className="py-20 max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(loading ? [{ name: "Basic", price: 500, duration_days: 7, is_featured: false }, { name: "Standard", price: 1200, duration_days: 15, is_featured: false }, { name: "Premium", price: 2500, duration_days: 30, is_featured: true }] : packages).map((pkg, i) => {
            ...
            return (
              <div key={i} className={`rounded-2xl p-8 relative transition-all ${isPremium ? "glow-blue" : ""}`}
                style={{
                  background: isPremium ? "linear-gradient(135deg, #1a2340, #1e2a4a)" : "rgba(20,27,45,0.8)",
                  border: isPremium ? "2px solid rgba(59,110,240,0.5)" : "1px solid rgba(59,110,240,0.15)",
                }}>
                {isPremium && (
                ...
                <ul className="space-y-3 mb-8">
                  {features.map((f, j) => (
                    <li key={j} className="flex items-center gap-2 text-sm text-slate-300">
                      <CheckCircle size={14} className="text-blue-400 flex-shrink-0" /> {f}
                    </li>
                  ))}
                </ul>
                <Link href="/register" className={`block w-full py-3 rounded-xl text-center font-semibold transition-all ${isPremium ? "btn-primary" : "btn-outline"}`}>
                  Get Started
                </Link>
              </div>
            );
          ...
          <div className="max-w-2xl mx-auto px-6">
            <div className="rounded-2xl p-6" style={{ background: "rgba(20,27,45,0.9)", border: "1px solid rgba(59,110,240,0.2)" }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="ml-auto px-2 py-0.5 rounded text-xs" style={{ background: "rgba(59,110,240,0.15)", color: "#5f8ef5" }}>
              ...
              {showAnswer ? (
                ...
                <button onClick={() => setShowAnswer(true)} className="px-5 py-2 rounded-xl text-sm font-medium btn-outline mb-4">
                  Show Answer
                </button>
              )}
              <button onClick={fetchNewQuestion} className="flex items-center gap-2 text-xs text-slate-500 hover:text-slate-300 transition-colors">
      ...
      <section className="mx-6 mb-20">
        <div className="max-w-7xl mx-auto rounded-3xl p-12 text-center relative overflow-hidden" style={{ background: "linear-gradient(135deg, #1a2340, #1e2a4a)", border: "1px solid rgba(59,110,240,0.3)" }}>
          ...
          <div className="flex flex-wrap justify-center gap-4 relative">
            <Link href="/register" className="px-8 py-4 rounded-2xl font-bold text-lg btn-primary glow-blue">
              Post Your Ad Now <ArrowRight size={20} className="inline ml-2" />
            </Link>
            <Link href="/explore" className="px-8 py-4 rounded-2xl font-semibold text-lg btn-outline">
              Browse Listings
            </Link>
          </div>
        </div>
